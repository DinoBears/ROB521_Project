...
def getAreaMaxContour(contours):
    ...
    for c in contours:  # 历遍所有轮廓 Traverse all contours
        ...
        if contour_area_temp > contour_area_max:
            ...
            if contour_area_temp > 300:  # 只有在面积大于300时，最大面积的轮廓才是有效的，以过滤干扰 Only when the area is greater than 300, the contour of the largest area is effective to filter interference
                area_max_contour = c

    return area_max_contour, contour_area_max  # 返回最大的轮廓 Return the largest contour

# 夹持器夹取时闭合的角度 The angle at which the gripper is closed when gripping
servo1 = 500
...
#设置扩展板的RGB灯颜色使其跟要追踪的颜色一致 Set the RGB light color of the expansion board to make it consistent with the color to be tracked
def set_rgb(color):
    if color == "red":
...
# app开始玩法调用 App start playing method call
def start():
    global __isRunning
    reset()
    __isRunning = True
...
# app停止玩法调用 app stop gameplay call
def stop():
    global _stop 
    global __isRunning
    _stop = True
...
world_X, world_Y = 0, 0
world_x, world_y = 0, 0
# 机械臂移动线程 Robotic arm moves thread
def move():
    global rect
    global track
    global _stop
    ...
    # 不同颜色木快放置坐标(x, y, z) Place coordinates of different colors(x, y, z)
    coordinate = {
    ...
    while True:
        if __isRunning:
            if first_move and start_pick_up: # 当首次检测到物体时  When an object is detected for the first time             
                ...
                setBuzzer(0.1)               
                result = AK.setPitchRangeMoving((world_X, world_Y - 2, 5), -90, -90, 0) # 不填运行时间参数，自适应运行时间 Do not fill in the running time parameters, adaptive running time
                if result == False:
                    unreachable = True
                else:
                    unreachable = False
                ...
                if track: # 如果是跟踪阶段 If it is the tracking stage
                    ...
                    time.sleep(0.02)                    
                    track = False
                if start_pick_up: #如果物体没有移动一段时间，开始夹取 If the object hasn’t moved for a while, start to grip
                    action_finish = False
                    if not __isRunning: # 停止以及退出标志位检测 Stop and exit flag detection
                        continue
                    Board.setBusServoPulse(1, servo1 - 280, 500)  # 爪子张开
                    # 计算夹持器需要旋转的角度 Calculate the angle that the gripper needs to rotate
                    ...
                    if not __isRunning:
                        continue
                    Board.setBusServoPulse(1, servo1, 500)  # 夹持器闭合 close grip
                    time.sleep(1)
                    
                    if not __isRunning:
                        continue
                    Board.setBusServoPulse(2, 500, 500)
                    AK.setPitchRangeMoving((world_X, world_Y, 12), -90, -90, 0, 1000)  # 机械臂抬起 Move the arm up
                    time.sleep(1)
                    
                    if not __isRunning:
                        continue
                    # 对不同颜色方块进行分类放置 Sort and place different colored squares
...
def run(img):
    ...
    #如果检测到某个区域有识别到的物体，则一直检测该区域直到没有为止 If an area is detected with a recognized object, it will continue to detect the area until there is none
    ...
    if not start_pick_up:
        ...
        if area_max > 2500:  # 有找到最大面积 Have found the largest area
            ...
            distance = math.sqrt(pow(world_x - last_x, 2) + pow(world_y - last_y, 2)) #对比上次坐标来判断是否移动 Compare the last coordinate to determine whether to move
